...
void Kernel::PrintInt(int number)
{
	...
	i--;
	while(i>=0){
	   synchConsoleOut->PutChar((char)(num[i--]+48));
	}
	putchar('\n');
}
...
int Kernel::Open(char *name)
{
	if(fileSystem->Open(name)==NULL)
		return -1;
	return (int)(fileSystem->Open(name));
}


int Kernel::Write(char *buffer, int size, int id)
{
    return (fileSystem->OpenFile->WriteAt(buffer, size, id));	
}


int Kernel::Close(int id)
{
	delete (OpenFileId)(id);
		return 1;
}
